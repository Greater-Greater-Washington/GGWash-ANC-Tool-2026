(function () {
  ...
  var CONFIG = {
    ...
    withdrawn: scriptBase + "data/withdrawn.csv",
  ...
  // Candidates who withdrew or were disqualified after the Aug 20 filing deadline but before
  // ballot certification -- they won't appear on the Nov. 3 ballot, but we keep their
  // questionnaire response (if any) on file rather than deleting it.
  function loadWithdrawn(rows) {
    var header = rows[0].map(function (h) { return norm(h).toLowerCase(); });
    var smdIdx = header.indexOf("smd");
    var nameIdx = header.indexOf("candidate name");
    var set = {};
    for (var i = 1; i < rows.length; i++) {
      var row = rows[i];
      if (!row || !norm(row[nameIdx])) continue;
      set[key(norm(row[smdIdx]), norm(row[nameIdx]))] = true;
    }
    return set;
  }

  function buildCandidates(rosterList, responseData, endorsementMap, withdrawnSet) {
    return rosterList.map(function (c) {
      ...
      return {
        ...
        withdrawn: !!(withdrawnSet && withdrawnSet[k])
      };
    });
  }
  ...
  // A few questions reference GGWash's indicator map by name (e.g. "You can see what parts of
  // the District those zones cover on our indicator map.") -- turn that phrase into an actual
  // link wherever a question's label is rendered, rather than leaving it as unlinked text.
  function renderQuestionLabel(label) {
    return escapeHTML(label).replace(/indicator map/gi, function (match) {
      return '<a href="https://ggwash.org/indicatormap/" target="_blank" rel="noopener">' + match + "</a>";
    });
  }
  ...
  function init(rootEl) {
    ...
    // "Last updated" reflects the repo's own most recent push, straight from the GitHub API, so
    // it stays accurate on every commit with no manual editing. Fails silently (leaves the line
    // blank) on network trouble or API rate-limiting, since it's a nice-to-have, not core to the
    // tool working.
    var updatedEl = rootEl.querySelector(".anc-updated");
    if (updatedEl) {
      fetch("https://api.github.com/repos/kaihallggwash/kai-anc-tool")
    ...
    Promise.all([
      ...
      var withdrawnSet = loadWithdrawn(results[5]);
      smdGeoJSON.features.forEach(function (f) { smdToAnc[norm(f.properties.SMD_ID).toUpperCase()] = f.properties.ANC_ID; });
      var candidates = buildCandidates(rosterList, responseData, endorsementMap, withdrawnSet);
      ...
      // Default string sort puts "3/4G01" before "3A01" (the "/" character sorts before
      // letters), throwing dual-ANC codes out of order. DC's two dual-ANC districts don't
      // follow one consistent rule -- GGWash's own ballot listing places 3/4G right after 3F
      // (within Ward 3's section) but puts 6/8F at the very end, after Ward 8, not with Ward 6
      // -- so these are matched explicitly rather than derived from a formula.
      var DUAL_ANC_SORT_WARD = { "3/4G": "3", "6/8F": "8" };
      function smdSortKey(id) {
        var upper = id.toUpperCase();
        var m = /^(\d+(?:\/\d+)?[A-Z]+)(\d+)$/.exec(upper);
        if (!m) return upper;
        var ancPart = m[1], num = m[2];
        var override = DUAL_ANC_SORT_WARD[ancPart];
        if (override) {
          var letters = ancPart.match(/[A-Z]+$/)[0];
          return override.padStart(2, "0") + letters + num.padStart(3, "0");
        }
        var wm = /^(\d+)([A-Z]+)$/.exec(ancPart);
        if (!wm) return upper;
        return wm[1].padStart(2, "0") + wm[2] + num.padStart(3, "0");
      }
      ...
      ).sort(function (a, b) { return smdSortKey(a) < smdSortKey(b) ? -1 : smdSortKey(a) > smdSortKey(b) ? 1 : 0; });
      ...
      function buildShareURL(smd, name) {
        var hash = "smd=" + encodeURIComponent(smd);
        if (name) hash += "&candidate=" + encodeURIComponent(name);
      ...
      function renderSelection(smdId) {
        ...
        selectionEl.querySelectorAll(".anc-share-btn").forEach(function (btn) {
          btn.addEventListener("click", function () {
            var url = buildShareURL(btn.getAttribute("data-smd"), btn.getAttribute("data-name"));
      ...
      // If the page loaded with a shared link (#smd=...&candidate=...), jump straight to that
      // SMD and, if a specific candidate was linked, open and scroll to their card.
      function applySharedLink() {
        if (!location.hash) return;
        var params = new URLSearchParams(location.hash.replace(/^#/, ""));
        var smd = params.get("smd");
        if (!smd) return;
        var matched = sortedSMDs.filter(function (id) { return id.toUpperCase() === smd.toUpperCase(); });
        if (!matched.length) return;
        selectSMD(matched[0]);
        var candidateName = params.get("candidate");
        if (!candidateName) return;
        setTimeout(function () {
          var cardEl = selectionEl.querySelector('.anc-candidate-card[data-name="' + CSS.escape(candidateName) + '"]');
          if (!cardEl) return;
          var details = cardEl.querySelector(".anc-responses");
      ...
      function withdrawnNote(c) {
        return c.hasResponse
          ? "This candidate will not appear on the Nov. 3, 2026, ballot, but we are maintaining a record of their responses to our questionnaire here."
          : "This candidate will not appear on the Nov. 3, 2026, ballot.";
      }

      function renderCompareView(list) {
        var respondents = list.filter(function (c) { return c.hasResponse; });
        var nonRespondents = list.filter(function (c) { return !c.hasResponse && !c.withdrawn; });
        var withdrawnCandidates = list.filter(function (c) { return c.withdrawn; });
        var html = "";
        ...
        withdrawnCandidates.forEach(function (c) {
          html += '<p class="anc-withdrawn-note">' + escapeHTML(c.name) + " — " + escapeHTML(withdrawnNote(c)) + "</p>";
        });
        ...
        questionOrder.forEach(function (q) {
          ...
          html += '<div class="anc-compare-question"><div class="anc-compare-q-text">' + renderQuestionLabel(meta.label) + '</div>';
      ...
      function renderCandidateCard(c, openByDefault) {
        var card = '<div class="anc-candidate-card' + (c.endorsed ? " endorsed" : "") + '" data-smd="' + escapeHTML(c.smd) + '" data-name="' + escapeHTML(c.name) + '">';
        ...
        card += '<button type="button" class="anc-share-btn" data-smd="' + escapeHTML(c.smd) + '" data-name="' + escapeHTML(c.name) + '">Copy link</button>';
        card += "</div>"; // .anc-candidate-headtext
        card += "</div>"; // .anc-candidate-header
        if (c.withdrawn) {
          card += '<p class="anc-withdrawn-note">' + escapeHTML(withdrawnNote(c)) + "</p>";
        }
        if (c.endorsed) {
        ...
        if (!c.hasResponse) {
          ...
          Object.keys(c.answers).forEach(function (q) {
            var val = c.answers[q];
            var meta = questionMeta(q);
            card += '<div class="anc-answer-row"><div class="anc-answer-q">' + renderQuestionLabel(meta.label) + '</div><div class="anc-answer-a">' + formatAnswerHTML(meta, val) + "</div></div>";
          });
          card += "</details>";
        }
        card += "</div>";
      ...
      function runSearch() {
        ...
        // Typing an SMD code directly works too (e.g. "1A01"), it's just not called out in the
        // placeholder since the dropdown already covers that use case explicitly.
        var qCompact = q.replace(/\s+/g, "").toUpperCase();
        var smdMatch = sortedSMDs.filter(function (id) { return id.toUpperCase() === qCompact; });
        if (smdMatch.length) {
          setStatus("");
          selectSMD(smdMatch[0]);
          return;
        }
